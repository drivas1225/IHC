...
public class NetworkCustom : NetworkManager
{
    ...
    public override void OnServerAddPlayer(NetworkConnection conn, short playerControllerId, NetworkReader extraMessageReader)
    {
        ...
        if (client.connection.playerControllers.Count != 0)
        {
            chosenCharacter = 1;
            GameObject spn = Instantiate(spawner);
            GameObject cnv = Instantiate(canvas);
            NetworkServer.Spawn(cnv);
            NetworkServer.Spawn(spn);
        }
